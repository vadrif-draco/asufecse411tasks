...
void LedTogglerTask_ISR_INIT(void) {
    GPIOIntRegister(GPIO_PORTF_BASE, LedTogglerTask_ISR);
...
int main(void) {
    PORTF_INIT();
    UART0_INIT(9600);
    LedTogglerTask_ISR_INIT();
    global_mutex = xSemaphoreCreateMutex();
    global_semaphore = xSemaphoreCreateBinary();
    if (global_mutex != NULL && global_semaphore != NULL) {
        xTaskCreate(LedTogglerTask, "Handler for SW2", 128, NULL, 3, NULL);
        xTaskCreate(CounterTask, "Continuous Printer", 128, NULL, 2, NULL);
        vTaskStartScheduler();
    }
    while (true);
}
